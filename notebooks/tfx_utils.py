...
class TFXReadonlyMetadataStore(utils.ReadonlyMetadataStore):
    ...
    def display_stats_for_examples(self, examples_id, split="Split-train"):
        ...
        if stats_artifact:
            stats_path = os.path.join(stats_artifact.uri, split, "FeatureStats.pb")
            print("READING", stats_path)
            tfdv.visualize_statistics(
                self.read_stats_proto(stats_path)
                # tfdv.load_statistics(stats_path)
            )

    def read_stats_proto(self, path) -> DatasetFeatureStatisticsList:
        with open(path, "rb") as f:
            saved_stats = DatasetFeatureStatisticsList()
            saved_stats.ParseFromString(f.read())
        return saved_stats
